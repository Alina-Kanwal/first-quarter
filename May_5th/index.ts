...
function halfryegg2(egg : number , salt : number, butter: number): number{
       return egg & salt & butter;
}
let result3 : number = halfryegg2(1 ,2 ,3)
console.log(result3)
...
function Division(Num : number , nuM : number){
      const conclusion = Num / nuM
      console.log("divsion: " + conclusion)
}
Division(4 ,5)
 
function addition(num : number , nuM : number){
     const conclusion = num + nuM
...
//RETURN FUNCTION
function message(x : string , y :string){
