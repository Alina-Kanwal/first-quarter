...
//    console.log(res)
// })
var currentUsers = ["BiLal", "HaMza", "Rafy", "JunNid", "TaHa"];
var new_users = ["Alina", "Tyyaba", "BiLal", "Ayesha", "TaHa"];
new_users.forEach(function (newusersname) {
    var lowercase = newusersname.toLowerCase(); //Array ki value ko exist karny kay lya for each ka tareeka
    if (currentUsers.map(function (c_users) { return c_users.toLowerCase(); }).includes(lowercase)) {
        console.log("The username ".concat(newusersname, " is not available. Please write a different username."));
    }
    else {
        console.log("The username ".concat(currentUsers, " is available."));
    }
});
